...
// config pseudo header
    ...
    while (1) {
        ...
        if ((size_t)recv_len < sizeof(struct iphdr))
            continue;
        ...
        if ((size_t)recv_len < ip->ihl * 4 + sizeof(struct tcphdr))
            continue;

        struct tcphdr *tcp =
            (struct tcphdr *)(buffer + ip->ihl * 4);

        if (ip->saddr != nmap.tip.s_addr)
            continue;

        if (ip->daddr != nmap.sip.s_addr)
            continue;

        if (tcp->source != nmap.pkt.tcp.dest)
            continue;

        if (tcp->dest != nmap.pkt.tcp.source)
            continue;

        printf("Bonne réponse reçue !\n");
